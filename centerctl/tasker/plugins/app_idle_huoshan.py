...
#coding:utf8
'''
火山app日活任务,总体任务调度不在这处理，只要考虑本任务即可
task原始任务格式
{
    'tid':"12344-1",
...
class AppIdleHuoshan(TaskerBase):
    
    act = 'app-idle-huoshan'
    ...
    def transfer_device(self):
        ...
        if self.dev_has_busy(devs):
            print "AppIdleHuoshan->Some devices busy!Wait..."
            return False
        ...
        for dev_info in devs:
            ...
            if not fake_attr_str or fake_attr_str == "":
                print "AppIdleHuoshan->No fake device attribute for cip:"+ip
                return False
    ...
    def transfer_net(self):
        ...
        for dev_info in devs:
            ...
            if not tast_act_dev:
                print "Task->AppIdleHuoshan:device task not finished,tid:{0}".format(tid)
                return False
        ...
        for dev_info in devs:
            ...
            if oarea:
                ...
                print "AppIdleHuoshan->oarea is None for cip:"+ip
    ...
    def tansfer_do(self):
        ...
        for dev_info in devs:
            ...
            if tast_act_dev and tast_act_dev != '0': #有任务没有完成,注意，这里统一认为vpn总是成功的!
                print "Task->AppIdleHuoshan:net task not finished,ip:{0}".format(ip)
                return False
    ...
    def transfer_done(self):
        ...
        for dev in devs:
            ...
            if do_status == None:
                print "Task->AppIdleHuoshan:appium task not finished,ip:{0}".format(ip)
                return False
        #TODO:对do任务结果进行处理
        for dev in devs:
            ip = dev["ip"]
    ...
    def report_task(self,tag,info):
        LOG.info("AppIdleHuoshan->action:report_task,tag:{0},info:{1}".format(tag,info)
