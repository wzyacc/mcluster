...
class AppCreateWeibo(TaskerBase):
    
    act = 'app-create-weibo'
    ...
    def transfer_device(self):
        ...
        if self.dev_has_busy(devs):
            LOG.info("AppCreateWeibo->Some devices busy!Wait...")
            return False
    ...
    def transfer_net(self):
        ...
        for dev_info in devs:
            ...
            if not tast_act_dev:
                LOG.info("Task->AppCreateWeibo:device task not finished,tid:{0}".format(tid))
                return False
    ...
    def tansfer_do(self):
        ...
        for dev_info in devs:
            ...
            if tast_act_dev and tast_act_dev != '0': #有任务没有完成,注意，这里统一认为vpn总是成功的!
                LOG.info("Task->AppCreateWeibo:net task not finished,ip:{0}".format(ip))
                return False
        ...
        if not users or len(users) == 0:
            ...
            self._rd.hset(cfg_rd_task,self._task["tid"],self._task)
            LOG.error("WeiboCreate->No unlogin user!We set task done for tid:{0}".format(self._task["tid"]))
            return False
    ...
    def transfer_done(self):
        ...
        for dev in devs:
            ...
            if do_status == None:
                LOG.info("Task->AppCreateWeibo:appium task not finished,ip:{0}".format(ip))
                return False
        #TODO:对do任务结果进行处理
        for dev in devs:
            ip = dev["ip"]
