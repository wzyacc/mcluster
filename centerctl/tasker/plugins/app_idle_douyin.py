...
#coding:utf8
'''
抖音app日活任务,总体任务调度不在这处理，只要考虑本任务即可
task原始任务格式
{
    'tid':"12344-1",
...
class AppIdleDouyin(TaskerBase):
    ...
    def transfer_device(self):
        ...
        if self.dev_has_busy(devs):
            print "AppIdleDouyin->Some devices busy!Wait..."
            return False
        ...
        for dev_info in devs:
            ...
            if not fake_attr_str or fake_attr_str == "":
                print "AppIdleDouyin->No fake device attribute for cip:"+ip
                return False
    ...
    def transfer_net(self):
        ...
        for dev_info in devs:
            ...
            if not tast_act_dev:
                print "Task->AppIdleDouyin:device task not finished,tid:{0}".format(tid)
                return False
        ...
        for dev_info in devs:
            ...
            if oarea:
                ...
                print "AppIdleDouyin->oarea is None for cip:"+ip
    ...
    def tansfer_do(self):
        ...
        for dev_info in devs:
            ...
            if tast_act_dev and tast_act_dev != '0': #有任务没有完成,注意，这里统一认为vpn总是成功的!
                print "Task->AppIdleDouyin:net task not finished,ip:{0}".format(ip)
                return False
    ...
    def transfer_done(self):
        ...
        for dev in devs:
            ...
            if do_status == None:
                print "Task->AppIdleDouyin:appium task not finished,ip:{0}".format(ip)
                return False
        #TODO:对do任务结果进行处理
        for dev in devs:
            ip = dev["ip"]
    ...
    def report_task(self,tag,info):
        LOG.info("AppIdleDouyin->action:report_task,tag:{0},info:{1}".format(tag,info))
