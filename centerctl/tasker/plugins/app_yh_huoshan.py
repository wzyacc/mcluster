...
class AppActiveHuoshan(TaskerBase):
    ...
    def transfer_device(self):
        ...
        if self.dev_has_busy(devs):
            LOG.info("YhHuoshan->Some devices busy!Wait...")
            return False
    ...
    def transfer_net(self):
        ...
        for dev_info in devs:
            ...
            if not tast_act_dev:
                LOG.info("Task->YhHuoshan:device task not finished,tid:{0}".format(tid))
                return False
    ...
    def tansfer_do(self):
        ...
        devs = self._rd.hget(cfg_rd_rdg,gid)
        if not devs:
            LOG.error("TaskManager->No devices for group gid:{0}".format(gid))
            return False
        devs = eval(devs) 
        ...
        for dev_info in devs:
            ...
            if tast_act_dev and tast_act_dev != '0': #有任务没有完成,注意，这里统一认为vpn总是成功的!
                LOG.info("Task->YhHuoshan:net task not finished,ip:{0}".format(ip))
                return False
    ...
    def transfer_done(self):
        ...
        for dev in devs:
            ...
            if do_status == None:
                LOG.info("Task->YhHuoshan:appium task not finished,ip:{0}".format(ip))
                return False
        #TODO:对do任务结果进行处理
        for dev in devs:
            ip = dev["ip"]
    ...
    def report_task(self,tag,info):
        LOG.info("YhHuoshan->action:report_task,tag:{0},info:{1}".format(tag,info))
