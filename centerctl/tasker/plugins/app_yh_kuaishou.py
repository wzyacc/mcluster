...
class AppActiveKuaishou(TaskerBase):
    
    act = 'app-active-kuaishou'
    ...
    def tansfer_do(self):
        ...
        for dev_info in devs:
            ...
            if tast_act_dev and tast_act_dev != '0': #有任务没有完成,注意，这里统一认为vpn总是成功的!
                print "Task->YhKuaishou:net task not finished,ip:{0}".format(ip)
                return False
        ...
        #给任务添加登录账号信息
        users = get_unlogin_user('kuaishou',len(devices))
    ...
    def transfer_done(self):
        ...
        for dev in devs:
            ...
            if do_status == None:
                print "Task->YhKuaishou:appium task not finished,ip:{0}".format(ip)
                return False
        #TODO:对do任务结果进行处理
        for dev in devs:
            ip = dev["ip"]
    ...
    def report_task(self,tag,info):
        LOG.info("YhKuaishou->action:report_task,tag:{0},info:{1}".format(tag,info))
