...
#coding:utf8
'''
秒拍app激活广告任务,总体任务调度不在这处理，只要考虑本任务即可
task原始任务格式
{
    'tid':"12344-1",
...
class AppActiveMiaopai(TaskerBase):
    
    act = 'app-active-miaopai'
    ...
    def transfer_device(self):
        ...
        if self.dev_has_busy(devs):
            print "AppActiveMiaopai->Some devices busy!Wait..."
            return False
    ...
    def transfer_net(self):
        ...
        for dev_info in devs:
            ...
            if not tast_act_dev:
                print "Task->AppActiveMiaopai:device task not finished,tid:{0}".format(tid)
                return False
    ...
    def tansfer_do(self):
        ...
        for dev_info in devs:
            ...
            if tast_act_dev and tast_act_dev != '0': #有任务没有完成,注意，这里统一认为vpn总是成功的!
                print "Task->AppActiveMiaopai:net task not finished,ip:{0}".format(ip)
                return False
    ...
    def transfer_done(self):
        ...
        for dev in devs:
            ...
            if do_status == None:
                print "Task->AppActiveMiaopai:appium task not finished,ip:{0}".format(ip)
                return False
        #TODO:对do任务结果进行处理
        ...
        while True:
            ...
            print "Not hit for miaopai-active for rd:{0},edge:{1}".format(rd,edge)
            #time.sleep(30)
            break #暂时取消分布
    ...
    def report_task(self,tag,info):
        LOG.info("AppAcitveMiaopai->action:report_task,tag:{0},info:{1}".format(tag,info))
