...
class AppActiveXiongmao(TaskerBase):
    
    act = 'app-active-xiongmao'
    ...
    def tansfer_do(self):
        ...
        #给任务添加登录账号信息
        users = get_unlogin_user('xiongmao',len(devices))
        if not users or len(users) == 0:
            self._task["status"] = 200
            self._rd.hset(cfg_rd_task,self._task["tid"],self._task)
            LOG.error("YhXiongmao->No unlogin user!We set task done for tid:{0}".format(self._task["tid"]))
            return False
    ...
    def report_task(self,tag,info):
        LOG.info("YhXiongmao->action:report_task,tag:{0},info:{1}".format(tag,info))
